...
def direccion(event):#Esto hace que no regrese por el mismo camino
    global direction
    if event == 'left':
        if direction != 'right':
            direction = event
        ...
        if direction != 'up':
            direction = event
def movimiento():#estas son las posiciones donde aparecera la manzana
    ...
    if posicion_food == posicion_snake[0]:
        ...
        #aca no se me carga la musica
        
        
        if posicion_food not in posicion_snake:#con esto la manzana no aparecera en la posicion de la snake
            canvas.coords(canvas.find_withtag("food"), posicion_food)
        canvas.create_text(*posicion_snake[-1], text= '▀', fill='green2', #creando a la serpiente
            font = ('Arial',20), tag ='snake')
    if posicion_snake[-1] == nueva_posicion[0] and len(posicion_snake)>=4: #aca es por si la serpiente se toca muere
        cruzar_snake()
